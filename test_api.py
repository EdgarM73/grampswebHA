...
try:
    ...
    print("Authenticating with Gramps Web API...")
    auth_resp = requests.post(f"{url}/api/token/", json={"username": username, "password": password}, timeout=10)
    if auth_resp.status_code != 200:
        print(f"Auth error: HTTP {auth_resp.status_code}")
    ...
    params = {"pagesize": 100, "sort": "birth"}
    response = requests.get(f"{url}/api/people/", params=params, headers=headers, timeout=10)
        ...
        if death_idx >= 0:
            ...
            if death_idx < len(events):
                ...
                if handle:
                    ...
                    try:
                        event_resp = requests.get(f"{url}/api/events/{handle}", headers=headers, timeout=10)
